...
const ProjectPage = () => {
  ...
  return (
    <div className="flex flex-col bg-white h-full bg-opacity-80 p-8">
      ...
      <ul ref={scrollRef} className="flex space-x-4 overflow-x-auto hide-scrollbar mb-6 shadow-xl border-2">
        {projectMetadata[projectNameKey]?.images.map((image, index) => (
          <li key={index} className="flex-shrink-0 w-1/4">
            <div className="relative h-96 cursor-pointer" onClick={() => setSelectedImage(image)}>
